...
namespace jags {
    ...
    double RmathRNG<_N01_kind, Derived>::exponential()
    {
        ...
        u = static_cast<Derived*>(this)->uniform();
        while(u <= 0.0 || u >= 1.0) u = static_cast<Derived*>(this)->uniform();
        for (;;) {
            u += u;
            if (u > 1.0)
                break;
        ...
        i = 0;
        ustar = static_cast<Derived*>(this)->uniform();
        umin = ustar;
        do {
            ustar = static_cast<Derived*>(this)->uniform();
            if (ustar < umin)
...
#define g(x)		(C1*exp(-x*x/2.0)-C2*(A-x))
        ...
        case  AHRENS_DIETER: /* see Reference above */
            ...
            u1 = static_cast<Derived*>(this)->uniform();
            s = 0.0;
            if (u1 > 0.5)
                s = 1.0;
            u1 = u1 + u1 - s;
                ...
                while (u2 <= t[i - 1]) {
                    ...
                    repeat {
                        ...
                        if (u2 < u1)
                            break;
                        tt = u1;
                        u2 = static_cast<Derived*>(this)->uniform();
                    }
                    u2 = static_cast<Derived*>(this)->uniform();
                }
                w = (u2 - t[i - 1]) * h[i - 1];
        ...
        case BOX_MULLER:
            if(_BM_norm_keep != 0.0) { /* An exact test is intentional */
                ...
                theta = 2 * PI * static_cast<Derived*>(this)->uniform();
                R = sqrt(-2 * log(static_cast<Derived*>(this)->uniform())) + 10*DBL_MIN; /* ensure non-zero */
        ...
        case KINDERMAN_RAMAGE: /* see Reference above */
            ...
            if(u1 >= 0.973310954173898) { /* tail: */
                repeat {
                    u2 = static_cast<Derived*>(this)->uniform();
                    u3 = static_cast<Derived*>(this)->uniform();
                    tt = (A*A-2*log(u3));
                    if( u2*u2<(A*A)/tt )
