...
#define R_Log1_Exp(x)   ((x) > -M_LN2 ? log(-rexpm1(x)) : log1p(-exp(x)))
    ...
    double bfrac(const double&, const double&, const double&, const double&, const double&, const double&);
    template<SCALE_P scale_p>
    void bgrat(const double&, const double&, const double&, const double&, double *, const double&, const int *);
    double grat_r(const double &a, const double &x, const double &r, const double &eps);
    double apser(const double&, const double&, const double&, double);
    template<SCALE_P scale_p>
    double bpser(const double&, const double&, const double&, const double&);
    template<SCALE_P scale_p>
    double basym(const double&, const double&, const double&, const double&);
    template<SCALE_P scale_p>
    double fpser(const double&, const double&, const double&, const double&);
    template<SCALE_P scale_p>
    double bup(const double&, const double&, const double&, const double&, const int&, const double&);
    double exparg(const int&);
    double psi(double&);
    double gam1(double&);
    double gamln1(double&);
    double betaln(const double&, double&);
    double algdiv(const double&, double&);
    template<SCALE_P scale_p>
    double brcmp1(const int&, const double&, const double&, const double&, const double&);
    template<SCALE_P scale_p>
    double brcomp(const double&, const double&, const double&, const double&);
    double rlog1(const double&);
    double bcorr(const double&, double);
    double gamln(const double&);
    double alnrel(const double&);
    template<SCALE_P scale_p>
    double esum(const int&, const double&);
    double erf__(const double&);
    double rexpm1(const double&);
    double erfc1(const int&, const double&);
    double gsumln(const double&, double);
...
#define SET_0_swap				\
	...
	if (min(a,b) <= 1.) { /*------------------------ a <= 1  or  b <= 1 ---- */
		...
		*w = fpser<scale_p>(a0, b0, x0, eps);
		*w1 = (scale_p==LOG_P) ? R_Log1_Exp(*w) : 0.5 - *w + 0.5;
		goto L_end;
	    }
		...
		*w1 = apser(a0, b0, x0, eps);
	    ...
	    n = 20; /* goto L130; */
	    *w1 = bup<NOR_P>(b0, a0, y0, x0, n, eps); did_bup = true;

	    b0 += n;
	L131:

	    bgrat<NOR_P>(b0, a0, y0, x0, w1, 15*eps, &ierr1);
...
//  !did_bup, i.e., where *w1 = (0 or -Inf) on entry
		if(did_bup) { // re-do that part on log scale:
		    *w1 = bup<LOG_P>(b0-n, a0, y0, x0, n, eps);
		}
		else *w1 = ML_NEGINF; // = 0 on log-scale
		bgrat<LOG_P>(b0, a0, y0, x0, w1, 15*eps, &ierr1);
		if(ierr1) *ierr = 8;
	    ...
	    *w = basym<scale_p>(a0, b0, lambda, eps * 100.0);
...
/*            EVALUATION OF THE APPROPRIATE ALGORITHM */
    ...
    L_w_bpser: // was L100
	*w = bpser<scale_p>(a0, b0, x0, eps);
	*w1 = (scale_p==LOG_P) ? R_Log1_Exp(*w) : 0.5 - *w + 0.5;
	goto L_end;

    L_w1_bpser:  // was L110
	*w1 = bpser<scale_p>(b0, a0, y0, eps);
	*w  = (scale_p==LOG_P) ? R_Log1_Exp(*w1) : 0.5 - *w1 + 0.5;
	goto L_end;

    L_bfrac:
	*w = bfrac<scale_p>(a0, b0, x0, y0, lambda, eps * 15.0);
	*w1 = log_p ? R_Log1_Exp(*w) : 0.5 - *w + 0.5;
	goto L_end;
	...
	*w = bup<NOR_P>(b0, a0, y0, x0, n, eps);
	...
	if (x0 <= 0.7) {
	    ...
	    *w += bpser<NOR_P>(a0, b0, x0, eps);
	    goto L_end_from_w;
	}
	/* L150: */
	if (a0 <= 15.0) {
	    n = 20;
	    *w += bup<NOR_P>(a0, b0, x0, y0, n, eps);
	    a0 += n;
	}
	bgrat<NOR_P>(a0, b0, x0, y0, w, 15*eps, &ierr1);
	if(ierr1) *ierr = 8;
	goto L_end_from_w;
...
/*                NOTE THAT 1/B(A,B) = B */
    ...
    double apser(const double &a, const double &b, const double &x, const double &eps)
    {
/* -----------------------------------------------------------------------
 ...
 * ----------------------------------------------------------------------- */
    ...
    double bpser<SCALE_P scale_p>(const double &a, const double &b, const double &x, const double &eps)
    {
/* -----------------------------------------------------------------------
 * Power SERies expansion for evaluating I_x(a,b) when
 *	       b <= 1 or b*x <= 0.7.   eps is the tolerance used.
 * ----------------------------------------------------------------------- */
	const bool log_p = (scale_p==LOG_P);
	int i, m;
...
/* ----------------------------------------------------------------------- */
	...
	if(fabs(w) > tol) { // the series did not converge (in time)
		...
		LOG_WARNING5("bpser did not converge");
	}
	if(log_p) {
    ...
    double bup<SCALE_P scale_p>(const double &a, const double &b, const double &x
				const double &y, const int &n, const double &eps)
    {
/* ----------------------------------------------------------------------- */
/*     EVALUATION OF I_x(A,B) - I_x(A+N,B) WHERE N IS A POSITIVE INT. */
/*     EPS IS THE TOLERANCE USED. */
/* ----------------------------------------------------------------------- */
	const bool give_log = (scale_p==LOG_P);
...
/* L30: */
    ...
    template<SCALE_P scale_p>
    double bfrac(const double &a, const double &b, const double &x, const double &y,
...
/*        CONTINUED FRACTION CALCULATION */
    ...
    template<SCALE_P scale_p>
    double brcomp(const double &a, const double &b, const double &x, const double &y)
    {
/* -----------------------------------------------------------------------
 *		 Evaluation of x^a * y^b / Beta(a,b)
 * ----------------------------------------------------------------------- */
	const log_p = (scale_p==LOG_P);
...
//                        -----
    template<SCALE_P scale_p>
    double brcmp1(const int &mu, const double &a, const double &b, const double &x, const double &y)
    {
	const bool give_log = (scale_p==LOG_P);
/* -----------------------------------------------------------------------
...
/* ----------------------------------------------------------------------- */
    ...
    template<SCALE_P scale_p>
    void bgrat(const double &a, const double &b, const double &x, const double &y,
	       double *w, const double &eps, int *ierr)
    {
	const bool log_w = (scale_p==LOG_P);
/* -----------------------------------------------------------------------
...
#define n_terms_bgrat 30
	...
	if (b * z == 0.0) { // should not happen, but does, e.g.,
	    ...
	    LOG_WARNING4("underflow, inaccurate pbeta()");
...
// called only from bgrat() , as   q_r = grat_r(b, z, log_r, eps)  :
    double grat_r(const double &a, const double &x, const double &log_r, const double &eps)
    {
/* -----------------------------------------------------------------------
 ...
 * ----------------------------------------------------------------------- */
    ...
    template<SCALE_P scale_p>
    double basym(const double &a, const double &b, const double &lambda, const double &eps)
    {
/* ----------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
	...
	int m = (l == 0) ? jags_i1mach(16) : jags_i1mach(15) - 1;
...
/*	  XMAX1	 = THE SMALLEST POSITIVE FLOATING POINT CONSTANT
	...
	d2 = 0.5 / jags_d1mach(3); /*= 0.5 / (0.5 * DBL_EPS) = 1/DBL_EPSILON = 2^52 */
