...
# define R_ifDEBUG_printf(...) JREprintf(__VA_ARGS__)
#else
 ...
 *      floating point number for which   1.0 + eps > 1.0 */
    eps = 2.0 * bnc_d1mach(3); /* == DBL_EPSILON (in R, Rmath) */
...
#ifdef DEBUG_bratio
	JREprintf(" ==> new w1=%.15g", *w1);
	if(ierr1) JREprintf(" ERROR(code=%d)\n", ierr1) ; else JREprintf("\n");
#endif
...
/* -------------------------------------------------------------------- */
    ...
    int m = (l == 0) ? bnc_i1mach(16) : bnc_i1mach(15) - 1;
 ...
 * Originally:  xmax1 = amin1(ipmpar(3), 1./spmpar(1))  */
    ...
    d2 = 0.5 / bnc_d1mach(3); /*= 0.5 / (0.5 * DBL_EPS) = 1/DBL_EPSILON = 2^52 */
