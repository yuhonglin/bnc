...
double qbeta(double alpha, double p, double q, int lower_tail, int log_p)
{
...
#ifdef DEBUG_qbeta
    BREprintf("qbeta(%g, %g, %g, lower_t=%d, log_p=%d):%s\n"
	     "  swap_tail=%d, la=%g: ",
	     alpha, p,q, lower_tail, log_p,
...
#ifdef DEBUG_qbeta
	BREprintf("p,q > 1 => w=%g", w);
#endif
	xinbta = pp / (pp + qq * exp(w + w));
    } else {
	r = qq + qq;
	t = 1. / (9. * qq);
	t = r * BR_pow_di(1. - t + y * sqrt(t), 3);
#ifdef DEBUG_qbeta
	BREprintf("min(p,q) <= 1: t=%g", t);
#endif
	if (t <= 0.) {
#ifdef DEBUG_qbeta
	    BREprintf(" t <= 0: log1p(-a)=%.15g\n", log1p(-a));
#endif
	    xinbta = 1. - exp((log1p(-a)+ log(qq) + logbeta) / qq);
	} else {
	    t = (4. * pp + r - 2.) / t;
#ifdef DEBUG_qbeta
	    BREprintf(" t >= 0:  new t = %g ( > 1 ?)\n", t);
#endif
	    if (t <= 1.)
    ...
    */
...
#ifdef DEBUG_qbeta
    BREprintf(" -> xinbta = %.16g (Newton acu=%g)\n", xinbta, acu);
#endif
	...
	g = 1;
#ifdef DEBUG_qbeta
	BREprintf("N(i=%d): x0=%.15g, pb(x0)=%.15g, w=%.15g, prev=%g,", i_pb, xinbta, y, w, prev);
#endif
...
#ifdef DEBUG_qbeta
			BREprintf(" it{in}=%d, delta(x)=%g, %s <= acu  ==> convergence\n",
