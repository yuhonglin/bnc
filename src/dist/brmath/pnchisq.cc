...
#ifdef DEBUG_pnch
	BREprintf("   pnchisq_raw(*, log_p): ans=%g => 2nd call, other tail\n", ans);
#endif
	    ...
	    Rboolean lower_tail, Rboolean log_p)
{
	...
	if(lower_tail && f > 0. &&
	   log(x) < M_LN2 + 2/f*(lgamma(f/2. + 1) + _dbl_min_exp)) {
	    ...
	    ans = sum - sum2;
#ifdef DEBUG_pnch
	    BREprintf("pnchisq(x=%g, f=%g, th.=%g); th. < 80, logspace: i=%d, ans=(sum=%g)-(sum2=%g)\n",
	    ...
	    ans = sum/sum2;
#ifdef DEBUG_pnch
	    BREprintf("pnchisq(x=%g, f=%g, theta=%g); theta < 80: i=%d, sum=%g, sum2=%g\n",
		     x,f,theta, i, (double)sum, (double)sum2);
#endif
    ...
    // else: theta == ncp >= 80 --------------------------------------------
#ifdef DEBUG_pnch
    BREprintf("pnchisq(x=%g, f=%g, theta=%g >= 80): ",x,f,theta);
#endif
...
#ifdef DEBUG_pnch
    BREprintf("-- v=exp(-th/2)=%g, x/2= %g, f/2= %g\n",v,x2,f2);
#endif
	...
	if (f_x_2n > 0) {
	    ...
	    if (((is_b = (bound <= errmax)) &&
            ...
            {
#ifdef DEBUG_pnch
                BREprintf("BREAK n=%d %s; bound= %g %s, rel.err= %g %s\n",
			 n, (is_it ? "> itrmax" : ""),
			 bound, (is_b ? "<= errmax" : ""),
...
#ifdef DEBUG_pnch_n
                BREprintf(" n=%d; nomore underflow in u = exp(lu) ==> change\n",
			 n);
#endif
	...
	if(tSml) {
		...
		/* no underflow anymore ==> change regime */
#ifdef DEBUG_pnch
                BREprintf("  n=%d; nomore underflow in t = exp(lt) ==> change\n", n);
#endif
