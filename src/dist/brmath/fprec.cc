...
/* Improvements by Martin Maechler, May 1997;
   ...
   Replace  pow(x, (double)i) by  BR_pow_di(x, i) {and use  int dig} */
...
double fprec(double x, double digits)
{
	...
	do_round = max10e - l10	 >= BR_pow_di(10., -dig);
	e2 = dig + ((e10>0)? 1 : -1) * MAX_DIGITS;
	p10 = BR_pow_di(10., e2);	x *= p10;
	P10 = BR_pow_di(10., e10-e2);	x *= P10;
	/*-- p10 * P10 = 10 ^ e10 */
	if(do_round) x += 0.5;
	x = floor(x) / p10;
	return(sgn*x/P10);
