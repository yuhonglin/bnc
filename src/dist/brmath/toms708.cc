...
# define R_ifDEBUG_printf(...) BREprintf(__VA_ARGS__)
#else
 ...
 *      floating point number for which   1.0 + eps > 1.0 */
    eps = 2.0 * br_d1mach(3); /* == DBL_EPSILON (in R, Rmath) */
...
#ifdef DEBUG_bratio
	BREprintf(" ==> new w1=%.15g", *w1);
	if(ierr1) BREprintf(" ERROR(code=%d)\n", ierr1) ; else BREprintf("\n");
#endif
...
/* -------------------------------------------------------------------- */
    ...
    int m = (l == 0) ? br_i1mach(16) : br_i1mach(15) - 1;
 ...
 * Originally:  xmax1 = amin1(ipmpar(3), 1./spmpar(1))  */
    ...
    d2 = 0.5 / br_d1mach(3); /*= 0.5 / (0.5 * DBL_EPS) = 1/DBL_EPSILON = 2^52 */
