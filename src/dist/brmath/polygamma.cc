 ...
 *    TOMS 9(4), pp. 494-502.
 *
 *    Routines called: br_d1mach, br_i1mach.
 */
...
void dpsifn(double x, int n, int kode, int m, double *ans, int *nz, int *ierr)
{
	...
	*ierr = 1;
	...
	if (n == 0)
	    ...
	    tt = -1/BR_pow_di(sin(x), 2);
	else if (n == 2)
	    tt = 2*cos(x)/BR_pow_di(sin(x), 3);
	else if (n == 3)
	    tt = -2*(2*BR_pow_di(cos(x), 2) + 1.)/BR_pow_di(sin(x), 4);
	else /* can not happen! */
	    tt = ML_NAN;
	/* end cheat */
    ...
    *nz = 0;
    ...
    mm = m;
    nx = imin2(-br_i1mach(15), br_i1mach(16));/* = 1021 */
    r1m5 = br_d1mach(5);
    r1m4 = br_d1mach(4) * 0.5;
