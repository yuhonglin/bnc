...
#ifdef DEBUG_qgamma
	BREprintf(" nu > .32: Wilson-Hilferty; x = %7g\n", x);
#endif
...
#ifdef DEBUG_qgamma
    BREprintf("qgamma(p=%7g, alpha=%7g, scale=%7g, l.t.=%2d, log_p=%2d): ",
...
#ifdef DEBUG_qgamma
	if(i == 1) BREprintf(" Ph.II iter; ch=%g, p2=%g\n", ch, p2);
	if(i >= 2) BREprintf("     it=%d,  ch=%g, p2=%g\n", i, ch, p2);
#endif
#ifdef IEEE_754
	if(!R_FINITE(p2) || ch <= 0)
#else
   ...
   */
	...
	for(i = 1; i <= max_it_Newton; i++) {
	    ...
	    if(i == 1) BREprintf("\n it=%d: p=%g, x = %g, p.=%g; p1=d{p}=%g\n",
				i, p, x, p_, p1);
	    if(i >= 2) BREprintf("          x{it= %d} = %g, p.=%g, p1=d{p}=%g\n",
				i,    x, p_, p1);
#endif
...
#ifdef DEBUG_q
		if(i == 1) BREprintf("no final Newton step because dgamma(*)== 0!\n");
#endif
		break;
	    }
	    /* else :
		...
		if(i == 1 && max_it_Newton > 1)
		    BREprintf("no Newton step done since delta{p} >= last delta\n");
#endif
		break;
	    } /* else : */
